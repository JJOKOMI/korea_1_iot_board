...
@RequiredArgsConstructor //생성자로 받겠다
public class ArticleController {
    ...
    public ResponseEntity<ResponseDto<ArticleResponseDto>> updateArticle(
            @AuthenticationPrincipal String userId, // 로그인되어있는지
            @PathVariable Long id, // 어떤아이디인지
            @RequestBody ArticleUpdateRequestDto dto // 어떤값으로 수정
    ) {
        Long authorId = Long.parseLong(userId);
    ...
    public ResponseEntity<ResponseDto<Void>> deleteArticle(
            @AuthenticationPrincipal String userId,
            @PathVariable Long id // 어떤아이디를 삭제할것인지
    ){
        Long authorId = Long.parseLong(userId);
    ...
    @GetMapping(ARTICLE_GET_BY_ID) // @AuthenticationPrincipal 애너테이션 없음 조회는 토큰필요없고 그냥보기 가능하기때문
    @PermitAll // @PermitAll : 인증여부와 관계없이 모든 사용자에게 접근을 허용
