...
/*
  ...
  <<기본 사용법>>
  react-cookie는 useCookies 훅을 통해 쿠키(cookies), 쿠키설정 함수(setCookie),
  쿠키제거함수(removeCookie)를 반환

  const [ cookies, setCookie, removeCookie] = useCookies(['쿠키이름']); //인자

  useCookies 훅에 전달되는 배열(인자값)
...
export default function D_react_cookie() {
  ...
  /*
    path: 쿠키가 유효한 경로를 지정
      - 주로'/'을 설정하여 모든 경로에서 유효하도록 쿠키를 설정

    expires: 쿠키의 만료 시간을 설정
      - 현재 시점부터 특정시간 경과 후 만료
    ...
    sameSite: 쿠키가 전송될 조건을 제한
      - strict 속성을 사용하여 다른사이트에서의 요청에서는 쿠키가 포함되지않는  
  */
