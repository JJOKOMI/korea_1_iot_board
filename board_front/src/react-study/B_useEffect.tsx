...
export default function UseEffect() {
  ...
  const handleToggleButtonClink = () => {
    setToggleData(prevData => !prevData);
  }
  
  // useEffect는 인자값
  // 첫번째 인자 (effect): 부수효과 - 통신(요청), 데이터 변경 시 동작될 효과
  // 두번째 인자 (deps): 의존성 배열 - 빈 배열일 경우 마운팅 시에만! 효과 발생
  //                    배열의 요소가 있는 경우 해당 값의 상태 변경 시 효과가 재발생
    ...
    <>
      ...
      <button onClick={handleButtonClick}>
        ...
        <div style={{
          ...
          <button onClick={handleToggleButtonClink}>상태변화버튼</button>
        </div>
      </div>
    </>
  )
